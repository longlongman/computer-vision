...
for i = 1 : length(t)
    ...
    b = [x(ceil(i/(d(2)+.1)),1);y(1,mod(i-1,d(2))+1);1];
    barycentric(i,:) = A \ b;
end
...
im1_morph = zeros(d);
for row_con = 1 : size(im1_morph,1)
    for col_con = 1 : size(im1_morph,2)
        if(isnan(x_morph(row_con,col_con))||isnan(y_morph(row_con,col_con)))
            continue;
        else
            im1_morph(row_con,col_con,:) = im1(round(x_morph(row_con,col_con)),round(y_morph(row_con,col_con)),:);
        end
    end
end
...
for row_con = 1 : size(im2_morph,1)
    for col_con = 1 : size(im2_morph,2)
        if(isnan(x_morph(row_con,col_con))||isnan(y_morph(row_con,col_con))) 
            continue;
        else
            im2_morph(row_con,col_con,:) = im2(round(x_morph(row_con,col_con)),round(y_morph(row_con,col_con)),:);
        end
    end
end
