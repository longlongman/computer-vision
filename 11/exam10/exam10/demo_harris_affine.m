...
%% 获取两幅图像
im1 = imread('mouse1.jpg');
im2 = imread('mouse2.jpg');
imwrite(rgb2gray(im1),'mouse1_gray.png');
imwrite(rgb2gray(im2),'mouse2_gray.png');
figure(1);imshow(cat(2, im1,im2));

%% 利用我们自己写的角点检测程序进行角点提取
% [ pos1, scale1, orient1, desc1 ] = SIFT(double(rgb2gray(im1)) / 255);
% [ pos2, scale2, orient2, desc2 ] = SIFT(double(rgb2gray(im2)) / 255);

[ pos1, scale1, orient1, desc1 ] = SiftMScale(double(rgb2gray(im1)) / 255,4,2);
[ pos2, scale2, orient2, desc2 ] = SiftMScale(double(rgb2gray(im2)) / 255,4,2);

% 提取图像的特征，为NCC服务
% descps1 = extractNccFeature(im1, pos1, 20);
% descps2 = extractNccFeature(im2, pos2, 20);
...
plot(pos1(:,1),pos1(:,2), 'ro', 'markersize',10);
plot(pos2(idxs2,1)+size(im1,2), pos2(idxs2,2), 'gx','markersize',10);

% for i = 1 : size(pos1,1)
%    plot([pos1(i,1), pos2(idxs2(i),1)+size(im1,2)],...
%         [pos1(i,2), pos2(idxs2(i),2)], 'r-');
% end
% 保存图像
...
% 用Ransac估计仿射变换
ntrials = 10000;
[A, inlineridxs] = est_optimal_Affine_ransac(pos2(idxs2,:), pos1,  ntrials);
...
for i = 1 : numel(inlineridxs)
   plot([pos1(inlineridxs(i),1), pos2(idxs2(inlineridxs(i)),1)+size(im1,2)],...
        [pos1(inlineridxs(i),2), pos2(idxs2(inlineridxs(i)),2)], 'ro','markersize',10);
end
% 保存图像
cdata = print('-RGBImage');
